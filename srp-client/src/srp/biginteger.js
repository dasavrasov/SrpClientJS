...
function GCD_(x, y) {
    ...
    while (sing) { //while y has nonzero elements other than y[0]
        ...
        if (!sing) {
            break;
        } //quit when y all zero elements except possibly y[0]
...
function addInt_(x, n) {
    ...
    for (i = 0; i < k; i++) {
        ...
        if (!c) {
            return;
        } //stop carrying as soon as the carry_ is zero
    }
}
...
function mont_(x, y, n, np) {
    ...
    for (i = 0; i < kn; i++) {
        ...
        //the inner "& mask" is needed on Macintosh MSIE, but not windows MSIE
        ui = ((t & mask) * np) & mask;
        c = (t + ui * n[0]) >> bpe;
        t = x[i];
...
export {add, str2bigInt,randBigInt,powMod, isZero, mod, bigInt2str, mult, greater, sub};
